...
<?php include('header_company.php'); ?>
        ...
        <div class="flex flex-1">
            ...
            <div class="container mx-auto my-10 p-10 bg-white rounded-lg shadow-md">
                <div class="flex flex-col space-y-4">
                    ...
                    if ($student_id !== null) {
                        // SQL query to fetch data for the specific application
                        $sql = "SELECT * FROM applicant WHERE student_id = $student_id";
                        ...
                        if ($result && $result->num_rows > 0) {
                            ...
                            echo "<span class='ml-2'>BL1-IC Form_LIM SHI KAI.pdf</span>";
                            echo "</div>";

                            echo "<div class='flex justify-between'>";
                            echo "<button class='button back-button' onclick='goBack()'>BACK</button>";
                            echo "<button class='button reject-button'
                            onclick='rejectApplication(" . $student_id . ")'>REJECT</button>";
                            echo "<button class='button approve-button'
                            onclick='acceptApplication(" . $student_id . ")'>APPROVE</button>";
                            echo "</div>";

                            echo "<p class='text-xs text-center text-gray-500 mt-4'>
                            PLEASE MAKE SURE APPROVE OR REJECT AFTER THE INTERVIEW SESSION DONE</p>";
                        } else {
    ...
    <script>
        ...
        function rejectApplication(studentId) {
            window.location.href = 'rejectApplication.php?student_id=' + studentId;
        }

        function acceptApplication(studentId) {
            window.location.href = 'acceptApplication.php?student_id=' + studentId
        }
    </script>
